# -*- coding: utf-8 -*-
"""CSE475-KMeansClustering.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1vuN_jiCoRcL7BUiugEXejyLDfcssknuy
"""
...
customer_data = pd.read_csv('./customers.csv')# YOUR CODE HERE
...
cust_shape = customer_data.shape# YOUR CODE HERE - Put the shape of the dataset here - 1.1 of Gradescope tests
print(cust_shape)
...
# drop='if_binary' creates 1 column for binary features, 2+ for others
x = encoder.fit_transform(customer_data[["Gender"]])
# x will now be shape (200, 1)

# 2. Create a temporary DataFrame for the new columns
# This gives them names like 'Gender_Female', 'Gender_Male'
encoded_df = pd.DataFrame(x, columns=encoder.get_feature_names_out(["Gender"]))

# 3. Join it back and drop the old string column
customer_data = pd.concat([customer_data.drop("Gender", axis=1), encoded_df], axis=1)

# Now corr() will work perfectly
...
matrix = customer_data.astype(float).corr()# YOUR CODE HERE
...
col_param_2 = "Spending Score (1-100)"# Put the name of the first column here - 1.3 of Gradescope tests

# What is the max positive value present in the matrix that is not equal to 1
matrix_max = .98 # 1.4 of Gradescope tests
print(matrix_max)
...
customer_data = customer_data[[col_param_1,  col_param_2]]# YOUR CODE HERE - subset the dataset for the 2 columns we are interested in
...
for i in range(1, num_clust + 1):
    ...
    kmeans = KMeans(n_clusters=i, init='k-means++', n_init=10, random_state=42) # write a KMeans instance with the following prescribed parameters - n_clusters = i (since we shall iterate over the multiple values of clusters we want to try out in the loop as defined above), init = 'k-means++', n_init = 10 and random_state = 42
...
kmeans = KMeans(n_clusters=chosen_cluster,
                init='k-means++',
                n_init=10,
                random_state=42)# write a KMeans instance with the following prescribed parameters - n_clusters = the chosen cluster value as reported above, init = 'k-means++', n_init = 10 and random_state = 42.fit(customer_data)
...
max_centre = kmeans.cluster_centers_[:, 0].max()
max_centre = max(kmeans.cluster_centers_[:, 1].max(), max_centre) # 1.6 of Gradescope tests
